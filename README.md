...
A Spring Boot backend that recreates the core product mechanics of [Lovable](https://lovable.dev) — an AI-powered app-builder platform. It handles user authentication, project & file management, team collaboration, subscription billing (Stripe), and per-user usage tracking, all behind a secured REST API.
...
2. Configure your database and JWT secret in `src/main/resources/application.yaml`, e.g.:
   ```yaml
   spring:
     application:
       name: lovable-clone
...
- Wire up an actual AI code-generation service for project scaffolding
- Add a live preview/sandbox runner (`Preview` entity is already modeled)
- WebSocket support for real-time chat-driven generation (`ChatSession` / `ChatMessage` entities are in place)
- Frontend client (React/Next.js) consuming this API

## License

No license specified yet — add one (e.g. MIT) if you intend to open this up for reuse.
